...
PROTECTED_PATHS=(
  ...
  "_works"
  "bower_components"
  # ── v1-leftover ROOT files: MOVED to PROTECTED_ROOT_FILES below ──
  # feed.xml / manifest.json / favicon.ico / browserconfig.xml used to live
  # here. They must NOT come back: --exclude-glob matches the BASENAME at any
  # depth, so a bare "feed.xml" here also blocked dist/newsletter/feed.xml
  # (task #1449). See PROTECTED_ROOT_FILES for the anchored replacement.
...
# ──────────────────────────────────────────────────────────────────────────
# PROTECTED_ROOT_FILES — v1-leftover files that live ONLY at the webroot and
# that the Astro build does not produce. Split out of PROTECTED_PATHS on
# 2026-07-20 (task #1449).
#
# WHY THE SPLIT. PROTECTED_PATHS entries become `--exclude-glob <name>`, and
# lftp's --exclude-glob matches the BASENAME at ANY DEPTH (verified with a
# local lftp 4.9.3 mirror, 2026-07-20). So "feed.xml" here did not just
# protect the root feed — it also silently dropped dist/newsletter/feed.xml
# from every break-glass run, the same defect that hit the CI workflows.
#
# These four are therefore emitted as ANCHORED REGEX excludes instead:
#   --exclude '^feed\.xml$'
# lftp's --exclude takes a regex matched against the path RELATIVE to the
# mirror root, so '^…$' pins it to the webroot top level only. Nested files
# of the same name (newsletter/feed.xml) now transfer normally, while the
# root originals stay protected from --delete exactly as before.
#
# NB the CI workflows cannot use this technique — multimatch there hard-codes
# matchBase:true, so they use a negated re-include (`!newsletter/feed.xml`)
# instead. Different mechanism, same net effect. Keep the two in sync.
#
# ADD A NEW ENTRY HERE only for files that exist at the webroot ROOT and are
# not emitted by the build. Anything that is a directory belongs in
# PROTECTED_PATHS above.
# ──────────────────────────────────────────────────────────────────────────
PROTECTED_ROOT_FILES=(
  "feed.xml"           # v1 Jekyll RSS feed; v2 HTML still <link>s to /feed.xml.
...
# --exclude takes a regex matched against the path relative to the mirror
# root, so '^name$' pins the exclude to the webroot top level and leaves
# same-named files at depth (newsletter/feed.xml) free to deploy. Dots are
# escaped so '.' cannot match an arbitrary character.
...
echo "==> PROTECTED root-only files (anchored, root level only):"
for p in "${PROTECTED_ROOT_FILES[@]}"; do echo "      - /${p}"; done
