...
#   ./astro/scripts/cutover.sh --archive-only   # runaway-workflow recovery: archive v1, skip v2-promote, remove /v2/
#   ./astro/scripts/cutover.sh --archive-only --dry-run
#
# --archive-only mode is for the scenario where deploy-prod.yml's first
# run completed before its cutover gate landed, so v2 contents are
# ALREADY at root and /v2/ is stale. We archive v1 leftovers and clean
# up /v2/, but skip the v2→root promotion step (would overwrite the
# good apex build with stale /v2/-base content).
...
# ──────────────────────────────────────────────────────────────────────────
# RUNAWAY-RECOVERY EXCLUSION LIST
#
# Used in --archive-only mode (Phase 7b runaway-workflow recovery). These
# v1 files would normally be archived in a clean cutover, but in the
# runaway scenario v2's build has already deployed to root and either:
#   - emits the same filename (e.g., index.html, 404.html — generated by
#     src/pages/*.astro) — archiving would remove v2's version from apex
#   - is referenced by v2's HTML <link>/<meta> tags but NOT shipped by v2
#     (manifest.json, favicon.ico, browserconfig.xml, feed.xml — v2's
#     HTML expects them at root, v1's leftover files serve them; archive
#     would break the references)
#
# In archive-only mode these stay at root. They get cleaned up via the
# deferred "Phase 7c webroot tidy" task (post-launch).
# ──────────────────────────────────────────────────────────────────────────
readonly RUNAWAY_KEEP_AT_ROOT=(
  "index.html"          # v2 src/pages/index.astro emits this
  "404.html"            # v2 src/pages/404.astro emits this
  "manifest.json"       # v2 HTML <link rel=manifest> references it
  "favicon.ico"         # v2 HTML <link rel=icon> references it
  "browserconfig.xml"   # v2 HTML <meta name=msapplication> references it
  "feed.xml"            # v2 HTML <link rel=alternate> references it
)
...
for arg in "$@"; do
  case "${arg}" in
    ...
    *)
      ...
      echo "Usage: $0 [--dry-run] [--rollback] [--archive-only]" >&2
      exit 2
      ;;
  esac
done

if (( ARCHIVE_ONLY )) && [[ "${MODE}" == "rollback" ]]; then
  echo "FATAL: --archive-only and --rollback are mutually exclusive." >&2
  exit 2
fi
...
do_cutover() {
  if (( ARCHIVE_ONLY )); then
    section "Phase 7b ARCHIVE-ONLY — runaway-recovery cleanup (no v2→root promotion)"
  else
  ...
  if (( ARCHIVE_ONLY )); then
    log "ARCHIVE-ONLY mode. Steps 1-2 (archive v1 leftovers) + Step 4 (rm /v2/) + Step 5 (smoke)."
    log "Skipping Step 3 (v2→root promotion) — v2 already at root from runaway workflow."
  fi
...
LFTP
  ...
  # Helper: in --archive-only mode, skip items v2 would overwrite or reference.
  should_skip_runaway() {
    local item="$1"
    (( ARCHIVE_ONLY )) || return 1
    local keep
    for keep in "${RUNAWAY_KEEP_AT_ROOT[@]}"; do
      [[ "${item}" == "${keep}" ]] && return 0
    done
    return 1
  }
  ...
  if (( ARCHIVE_ONLY )); then
    section "Step 3 — SKIPPED (--archive-only mode; v2 already at root)"
    log "Skipping v2→root promotion. v2's contents are already at apex root."
    log "from the runaway workflow that ran before the cutover gate landed."
  else
...
LFTP
  ...
  # ── Step 4: remove v2/ ───────────────────────────────────────────────
  if (( ARCHIVE_ONLY )); then
    section "Step 4 — SKIPPED (--archive-only mode; /v2/ still has full build inside)"
    log "Skipping /v2/ removal. Run separately after the cutover gate is satisfied:"
    log "  lftp ... <<< 'rm -r ${REMOTE_V2}'"
    log "Or use the runbook's post-archive cleanup heredoc."
  else
...
LFTP
  ...
  # ── Done banner ──────────────────────────────────────────────────────
  if (( ARCHIVE_ONLY )); then
    section "Archive-only complete"
    if (( DRY_RUN )); then
      echo "DRY-RUN summary: ${OPS_PLANNED} ops would run."
      echo ""
      echo "When you're ready, run WITHOUT --dry-run:"
      echo "    ./scripts/cutover.sh --archive-only"
    else
      echo "v1 archive complete. ${OPS_RUN}/${OPS_PLANNED} ops executed."
      echo ""
      echo "    Public:    ${APEX_URL}  (serving new build, unchanged)"
      echo "    Admin:     ${APEX_ADMIN_URL}"
      echo "    Archive:   public_html/${REMOTE_ARCHIVE}/  (30-day rollback window)"
      echo "    Stale v2:  public_html/${REMOTE_V2}/  (cleanup separately)"
      echo ""
      echo "Next steps:"
      echo "  1. (Optional) remove stale /v2/ tree:"
      echo "       lftp ... rm -r ${REMOTE_V2}"
      echo "  2. Flip Sveltia config.yml site_url to apex (separate commit)"
      echo "  3. Browser smoke test from incognito session"
      echo ""
      echo "Rollback is unsafe in archive-only mode (would expect v2 in /v2/, not at root)."
      echo "If apex is broken, contact Cord — recovery is custom from this state."
    fi
  else
